...
interface HeaderWithChildren extends DefaultTheme.Header {
  children?: Header[]
}

export const ContentLink: FunctionalComponent<{
  ...
  const maxDepth = frontmatter.value.tocDepth || Infinity
  ...
  return h('li', { class: 'sidebar-link' }, [
    h(
      link ? 'a' : 'p',
      {
        class: { 'sidebar-link-item py-1 transition-colors hover:bg-gray-100 duration-150': true, active },
        href: link
      },
      text
    ),
...
function createChildren(
  ...
  if (children && children.length > 0) {
    return h(
      'ul',
      { class: 'sidebar-links pl-2' },
      children.map((c) => {
        return h(ContentLink, { item: c, depth })
      })
    )
  }
